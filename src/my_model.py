...
from modules.transformer_decoder import DecoderTransformer, Quantity_TF
...
def get_model(args, ingr_vocab_size, instrs_vocab_size):
    ...
    if not args.train_ingr_only:
        quantity_decoder = Quantity_TF(embed_size = args.embed_size, num_ingredients = ingr_vocab_size, num_layers=args.transf_layers_quantity)
        quantity_loss = nn.MSELoss(reduce = False)
    else:
        quantity_decoder = None
        quantity_loss = None

    if args.semantic:
        semantic_branch = nn.Linear(args.embed_size, args.n_classes) ## numClasses = 1048
        ...
        semantic_crit = nn.CrossEntropyLoss(weight=weights_class).to(device)
    else:
        semantic_branch = None
        semantic_crit = None

    model = InverseCookingModel(encoder_ingrs, decoder, ingr_decoder, encoder_image, quantity_decoder, semantic_branch,
                                crit=criterion, crit_ingr=label_loss, crit_eos=eos_loss, crit_quantity = quantity_loss, crit_semantic = semantic_crit,
...
class InverseCookingModel(nn.Module):
    def __init__(self, ingredient_encoder, recipe_decoder, ingr_decoder, image_encoder, quantity_decoder, semantic_branch,
                 crit=None, crit_ingr=None, crit_eos=None, crit_quantity=None, crit_semantic=None,
        ...
        self.crit_semantic = crit_semantic
    ...
    def forward(self, img_inputs, captions=None, target_ingrs=None, quantity_gt = None, class_gt=None,
        ...
        losses = {}
        ## quantity prediction 
        if self.quantity_decoder is not None:
            quantity_pred = self.quantity_decoder(img_features)
            quantity_loss = self.crit_quantity(quantity_pred, quantity_gt)
            losses['quantity_loss'] = quantity_loss

        ## semantic prediction #####
        if self.semantic_branch is not None:
            pool = self.pool(img_features).squeeze(-1)
            class_pred = self.semantic_branch(pool)
            semantic_loss = self.crit_semantic(class_pred, class_gt.squeeze(-1))
    ...
    def sample(self, img_inputs, greedy=True, temperature=1.0, beam=-1, true_ingrs=None):
        ...
        if self.quantity_decoder is not None:
            quantity_pred = self.quantity_decoder(img_features)
            selected_ids = ingr_ids[sample_mask != 0]
            quantity_sample = quantity_pred[selected_ids]
            outputs['quantity_sample'] = quantity_sample
